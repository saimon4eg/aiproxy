...
// ParseProxyURL parses a proxy URL string. If no scheme is present, prepends "http://".
// Supports http://, socks5://, socks5h://.
...
// SetTransportProxy configures the transport to route through the given proxy.
// Pass empty string to disable.
func SetTransportProxy(t *http.Transport, proxyHost string) {
	if proxyHost == "" {
		t.Proxy = nil
		return
	}
	proxyURL, err := ParseProxyURL(proxyHost)
	if err != nil {
		slog.Warn("invalid proxy_host, ignoring", "value", proxyHost, "error", err)
		return
	}
	t.Proxy = http.ProxyURL(proxyURL)
	slog.Info("copilot upstream proxy configured", "proxy", proxyURL.String())
}
...
// parseProxyURL parses a proxy URL string, prepending "http://" if no scheme is present.
func parseProxyURL(raw string) (*url.URL, error) {
	if !strings.Contains(raw, "://") {
		raw = "http://" + raw
	}
