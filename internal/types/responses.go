...
type ResponseOutputContent struct {
	...
	Text    string `json:"text"` // must serialize even when empty for Codex compatibility
...
// McpToolFullNames maps short MCP tool names to their full namespaced names
// (e.g., "navigate_page" → "mcp__chrome_devtools_mcp__navigate_page").
// Populated during tool normalization, used during stream conversion.
var (
	McpToolFullNames   = make(map[string]string)
	mcpToolFullNamesMu sync.RWMutex
)

// RegisterMcpToolName registers a short-to-full name mapping.
func RegisterMcpToolName(shortName, fullName string) {
	mcpToolFullNamesMu.Lock()
	McpToolFullNames[shortName] = fullName
	mcpToolFullNamesMu.Unlock()
}

// ResolveMcpToolFullName returns the full namespaced name or empty string.
func ResolveMcpToolFullName(shortName string) string {
	mcpToolFullNamesMu.RLock()
	defer mcpToolFullNamesMu.RUnlock()
	return McpToolFullNames[shortName]
}

// ResolveMcpToolNamespace returns just the namespace portion of the full
// namespaced name (e.g., for "mcp__chrome_devtools_mcp__navigate_page"
// returns "mcp__chrome_devtools_mcp__"). Used to set the namespace field
// on function_call output items so Codex can route MCP calls.
func ResolveMcpToolNamespace(shortName string) string {
	mcpToolFullNamesMu.RLock()
	full := McpToolFullNames[shortName]
	mcpToolFullNamesMu.RUnlock()
	if full == "" {
		return ""
	}
	// Full name format: "mcp__server__toolname".
	// Find the LAST "__" (before tool name) and include it in the namespace.
	lastIdx := strings.LastIndex(full, "__")
	if lastIdx < 0 {
		return ""
	}
	return full[:lastIdx+2] // include the "__" separator
}
