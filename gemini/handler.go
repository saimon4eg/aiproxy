...
func (h *Handler) handleStreamingGenerateContent(w http.ResponseWriter, r *http.Request, openAIReq types.OpenAIChatCompletionsRequest, targetEndpoint string) {
	var reqBody interface{} = openAIReq
	if targetEndpoint == "/responses" {
		reqBody = proxy.ConvertChatToResponsesRequest(openAIReq)
	}
	...
	var chatStream io.Reader = resp.Body
	if targetEndpoint == "/responses" {
		pr, pw := io.Pipe()
		defer pr.Close()
		go func() {
			pw.CloseWithError(proxy.StreamResponsesAsChatChunks(pw, resp.Body))
		}()
		chatStream = pr
	}
	if err := streamChatAsGemini(w, chatStream); err != nil {
