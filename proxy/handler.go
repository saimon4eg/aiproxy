...
func NewHandler(authClient *auth.Client, transport http.RoundTripper, mc *models.Cache, debug bool) *Handler {
	return &Handler{
...
// upstreamEndpoint expands the internal endpoint path to the full upstream path.
// proxyHandler strips /v1 from incoming paths; some endpoints need it back.
func upstreamEndpoint(endpoint string) string {
